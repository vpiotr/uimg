...
public:
    ...
    virtual void putPixel(unsigned int x, unsigned int y, const RgbColor &color) {
        target_.setPixel(Point(UNSIGNED_CAST(int, x), UNSIGNED_CAST(int, y)), color);
    }
...
public:
    ...
    virtual void putPixel(unsigned int x, unsigned int y, const RgbColor &color) {
        ...
        unsigned char *dataPtr = UNSIGNED_CAST(unsigned char *, image_.data()) + offset;
        *(dataPtr++) = color.red;
        *(dataPtr++) = color.green;
        *(dataPtr++) = color.blue;
    }
...
protected:
    ...
    virtual void drawHorizontalLine(unsigned int x1, unsigned int x2, unsigned int y, const RgbColor &color) {
        ...
        int x1c = UNSIGNED_CAST(int, std::min(x1, image_.width() - 1));
        int x2c = UNSIGNED_CAST(int, std::min(x2, image_.width() - 1));
...
public:
    ...
    drawFull(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2, const RgbColor &color) {
        ...
        unsigned char *lnPtr = UNSIGNED_CAST(unsigned char *, image_.data()) + lnOffset;
...
public:
    ...
    virtual void paint(const RgbColor &color) {
        ...
        unsigned char *data = UNSIGNED_CAST(unsigned char *, image_->data());
