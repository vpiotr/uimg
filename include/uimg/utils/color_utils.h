...
public:
    ...
    static RgbColor mix_colors(const RgbColor &color1, const RgbColor &color2, float partOfColor1) {
        ...
        result.red = static_cast<unsigned char>(std::min(255, std::max(0, static_cast<int>(round(color1.red * factor1 + color2.red * factor2)))));
        result.green = static_cast<unsigned char>(std::min(255,
                                std::max(0, static_cast<int>(round(color1.green * factor1 + color2.green * factor2)))));
        result.blue = static_cast<unsigned char>(std::min(255,
